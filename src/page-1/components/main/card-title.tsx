...
export function CreateCardTitle(props: BodyWrapperProps) {
	return (
		<div className="main-title-wrapper">
			<div className="title-wrapper flex justify-between gap-6 px-7 py-4 sm:gap-4 sm:px-5">
				<a href={props.cardWrapper.url}>
					<h1 className="title text-[22px] text-[#1f1513] lg:text-[18px] xl:text-[22px] font-['Commissioner'] font-bold leading-[25px] hover:text-[#9B7C5F] transition-all duration-400">{ props.cardWrapper.title }</h1>
				</a>
				<div className="hh-link-wrapper flex justify-center bg-[#f5f5f5] hover:bg-white hover:shadow-myInner hover:shadow-[#bdbdbd] min-w-[80px] h-[48px] lg:min-w-[50px] xl:min-w-[80px] rounded-2xl shadow-md transition-all duration-400">
					<a href={props.cardWrapper.url} className="flex items-center gap-3">
						...
						<p className="lg:hidden xl:flex font-['Commissioner'] font-bold text-[16px] text-[#616161]">&gt;</p>
					</a>
				</div>
			</div>
			<div className="bg-mainBg rounded-sm flex justify-center uppercase lg:text-[12px] py-1 font-['Commissioner'] font-normal text-[#616161]">
				<p>{props.cardWrapper.category}</p>
			</div>
		</div>
	)
}